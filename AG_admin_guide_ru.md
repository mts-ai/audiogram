...
Audiogram позволяет распознавать и синтезировать аудио в форматах:

* **LINEAR_PCM** — несжатый аудиоформат, в котором звуковая волна оцифровывается напрямую без потери качества;
* **MULAW** — алгоритм компрессии аудио (компандирования) (стандарт G.711u) для эффективной передачи речи с уменьшением полосы пропускания;
* **ALAW** — европейский стандарт компрессии аудио (G.711a), аналогичный Mu-law, оптимизированный для цифровых телефонных сетей с целью экономии трафика при сохранении разборчивости человеческой речи.

Характеристики аудио для форматов приведены в таблице:

| **Формат** | **Параметры** | **Контейнер** | **Транскодер** |
| ------ | ------- | ------ | ------- | 
| LINEAR_PCM ("родной формат") | 16-бит, signed,</br> little-endian,</br> 16000 Гц, моно | WAV или без контейнера (формат указывается в конфиге запроса)| Не требуется — обрабатывается нативно |
| MULAW | По стандарту G.711 (μ-law/PCMU) | Без контейнера | Обязателен — приводится к LINEAR_PCM 16 кГц/моно перед распознаванием  |
| ALAW | По стандарту G.711 (A-law/PCMA) | Без контейнера | Обязателен — приводится к LINEAR_PCM 16 кГц/моно перед распознаванием  |

Формат аудиоданных (кодировку) можно задать при вызове методов распознавания и синтеза речи в  поле **encoding**, которое возвращает тип перечисления **AudioEncoding**. Смотрите раздел **Справочник API**. 
...
| 6 | common | etcd | CPU | Используется в качестве service-registry. service-registry - service discovery для приложений tts-agent/asr-e2e-agent.  |
...
**Формат передачи данных (dictionary_data)**</br>
