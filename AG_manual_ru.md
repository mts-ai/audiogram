...
## Текущая версия - Audiogram 3.44.0

Обновление Audiogram 3.44.0 содержит следующие улучшения, исправления и изменения:

**Изменения**
* Добавлен новый сервис экосистемы IAM v4 — PAP (Policy Administration Point) - он предоставляет GraphQL API для взаимодействия с базой данных IAM и выполнения атомарных операций.
* Добавлен новый сервис экосистемы IAM v4 — PDP (Policy Decision Point) - он предоставляет gRPC интерфейс для выполнения операций авторизации, проверки прав и множественной проверки прав.
* Добавлен новый сервис admin-backend - он предоставляет HTTP REST API и выступает в качестве прокси-сервиса над PAP.

**Улучшения**
* В компонентах api для синтеза и распознавания речи:
     * Добавлена поддержка балансировщика gRPC с параметром round-robin.
     * Исправлены названия resource и action для авторизации и проверки доступа через сервис PDP в соответсвии с параметрами обновленных политик для IAM v4.

## Предыдущие версии

### Audiogram 3.42.0

Audiogram 3.42.0 содержит следующие улучшения:
* Добавлено сжатие данных с метриками для компонентов tts-agent, asr-e2e-agent и vad-agent.
* Сделана доработка клиента для транскодера для работы с gRPC-стримами в едином режиме.
* В ряде сервисов к контексту логирования добавлены поля client_id и trace_id.

### Audiogram 3.41.0

Audiogram 3.41.0 содержит следующие улучшения:
* В компоненте препроцессинга синтеза речи (transcriber):
     * Обновлена версия Python.
     * Внесены улучшения по результатам тестирования пользователями.
* В компоненте препроцессинга синтеза речи (acronorm):
     * Исправлена ошибка с озвучкой тегов.
     * Исправлена ошибка с влиянием комбинации ```\n``` на обработку SSML-тегов.
     * Исправлена ошибка с озвучкой отрицательных чисел при использовании SSML-тега ```<money>```.
     * Исправлена ошибка с озвучкой слов, оканчивающихся на звук "с" (например, страниц).
     * Исправлена ошибка с озвучкой больших чисел по одной цифре.
* В компоненте speech-segmentation-server исправлена ошибка, возникшая после перехода на uuid4 - под большой нагрузкой обрабатывались не все запросы, что приводило к падению сервера.
* Обновлены компоненты медиа-архива - исправлены проблемы с накоплением и обработкой очереди запросов.

### Audiogram 3.40.0

Audiogram 3.40.0 содержит следующие улучшения:
* В компонент vad-agent добавлена возможность работать с STR- и INT-типом sequence_id.
* Реализовано логирование сущностей client_id и trace_id, переданных через метаданные.
* Обновлена индексация запросов в asr-e2e-agent с тем, чтобы в частном случае индексации пустых финальных гипотез выставляемые индексы совпадали в компонентах asr-e2e-agent и api.
* Исправлена ошибка с передачей метки begin при отключенном VAD и включенном VA_response_mode в потоковом режиме.
* Добавлено поле rt_lag к событию по отправке финальной гипотезы.
* Компонент grpc-gateway добавлен в основную поставку.
* В компоненте grpc-gateway реализована работа с отложенным распознаванием через HTTP.

### Audiogram 3.39.0

Audiogram 3.39.0 содержит следующие улучшения:
* Обновлена модель распознавания речи e2e-v3 - в нее добавлен адаптер, улучшающий работу в домене телекоммуникаций, за счет чего повышается качество распознавания и снижается WER.
* Обновлен сервис постпроцессинга распознавания речи - для оптимизации работы сервера теперь используется кастомный Triton для TensorRT LLM.
* Обновлена модель диаризации спикеров в составе компонента speech-segmentation-server и соответствующий агент:
     * Поддержан режим отложенного распознавания — теперь Audiogram может распознавать файлы длиной до 12 часов и количеством спикеров до 20.
     * Оптимизирована система обработки меток VAD внутри алгоритма диаризации для более корректного сбора и обработки потоковых меток.
     * Исправлена фрагментация спикеров на границе двух чанков.
* Добавлена расширенная трассировка для модели Voice Activity Detection.
* Исправлена ошибка с неверной расстановкой временнЫх меток в распознавании речи.

**Улучшения**
* Обновлен компонент acronorm (обработчик сокращений + нормализатор чисел):

### Audiogram 3.38.1

Audiogram 3.38.1 содержит следующие улучшения:

**Улучшения**
* Обновлен компонент acronorm (обработчик сокращений + нормализатор чисел):
     * Исправлена ошибка в SSML-теге ```say-as <money>```, приводящая к падению сервера в случае синтеза чисел, целая или дробная часть которых оканчивается на 1.
     * Добавлена поддержка отрицательных чисел в SSML-теге ```say-as <money>```.
     * Исправлена неверная вербализация 0 в атрибуте ordinal.
     * Иcправлены неверная логика перечислений и интервалов для атрибута decimal.
     * Сделаны доработки по результатам тестов пользователей.
     * Обновлена версия Triton-сервера для оптимизации работы компонента.
* Обновлены компоненты медиа-архива receiver и receiver-balancer - справлены ошибки с прекращением записи в архив, переполнением очереди и неоптимальным использованием файлового хранилища.

### Audiogram 3.38.0

Audiogram 3.38.0 содержит следующие улучшения и исправления:

**Новые возможности**
* Добавлена поддержка клонирования голоса. Реализация включает два новых компонента:
     * Сервис эмбеддингов, который извлекает вектор голоса пользователя из аудиозаписи.
     * Сервис клонирования, который позволяет осуществлять синтез голосом пользователя.

**Улучшения**
* Обновлен сервис сегментации речи (speech-segmentation-server) — улучшена модель выделения целевой речи, добавлена возможность ее работы на чанках меньшего размера.
* Проведено улучшение работы компонента transcriber:
     * Реализована следующая итерация доработок по контекстным омографам на Ë.
     * Исправлена проблема с вырезанием закрывающих тегов speak и emphasis.
     * Исправлен частный случай омографии в словах (дОма — домА).
     * Улучшена обработка аббревиатур.
     * Исправлены эпизодические случаи смягчения согласных перед твердым знаком.

### Audiogram 3.37.0

Audiogram 3.37.0 содержит следующие улучшения, исправления и изменения:
