...
При написании приложения используйте [proto-файл **stt.proto**](https://github.com/mts-ai/audiogram/blob/main/asr/v3/stt_v3.proto).

Максимальная длина сообщения, принимаемого от клиентов по gRPC (в байтах): 62914560
...
| :--- | :--- | :--- | :--- |
| FileRecognize | [FileRecognizeRequest](#filerecognizerequest) | [FileRecognizeResponse](#filerecognizeresponse) | Метод распознавания аудиофайла целиком. Ожидает аудиофайл, в этом же соединении возвращает результат и закрывает соединение. |
...
| Имя метода | Тип запроса | Тип ответа | Описание |
| :--- | :--- | :--- | :--- |
| Recognize | stream [RecognizeRequest](#recognizerequest) | stream [RecognizeResponse](#recognizeresponse) | Этот метод позволяет проводить распознавание речи в потоковом режиме. Клиент устанавливает соединение с Audiogram и отправляет аудио на распознавание по кускам (чанками). Рекомендуется делать длительность чанка от 250 до 500 миллисекунд (мс). Распознавание проходит по мере поступления аудиоданных и заканчивается, когда поток закрывается клиентом. |
...
//Начало конфига запроса по распознаванию речи
...
Первое сообщение типа [RecognizeRequest](#recognizerequest) должно содержать данные в поле «config» и не должно содержать данные в поле «audio». Все последующие сообщения [RecognizeRequest](#recognizerequest), наоборот, не должны иметь данных в поле «config», а в поле «audio» передаются аудиоданные. Аудиобайты должны быть закодированы, как указано в [RecognitionConfig](#recognitionconfig).

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| config | [StreamRecognitionConfig](#streamrecognitionconfig) | Конфигурация распознания. |
...
Конфигурация распознавания при вызове метода [FileRecognize](#filerecognize).

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| encoding | enum [AudioEncoding](#audioencoding) | Формат аудиоданных (кодировка). |
...
| enable_word_time_offsets | bool | Флаг, включающий вывод временных меток слов (которые возвращаются в поле [word](#WORD) структуры [WordInfo](#wordinfo)).<br> <li>при значении «true» результат включает временные метки для этих слов;<br> <li>при значении «false» информация о временных метках не возвращается. |
...
| genderage_config | [GenderAgeEmotionConfig](#genderageemotionconfig) | Конфигурация модели по определению пола, возраста и эмоционального настроя говорящего. |
| antispoofing_config | [AntiSpoofingConfig](#antispoofingconfig) | Конфигурация антиспуфинга. |
| context_dictionary | [ContextDictionaryConfig](#contextdictionaryconfig) | Конфигурация распознавания речи с участием словаря. |
| punctuation_config | [PunctuationConfig](#punctuationconfig) | Конфигурация модели проставления пунктуации. |
| denormalization_config | [DenormalizationConfig](#denormalizationconfig) | Конфигурация модели денормализации чисел (перевод текстового представления числа в цифровую, например, "пять объектов > 5 объектов"). |
| speaker_labeling_config | [SpeakerLabelingConfig](#speakerlabelingconfig) | Конфигурация диаризации (определение принадлежности распознаваемых фраз к определенным спикерам и разделение аудио на сегменты, относящиеся к определенным спикерам) |
...
| mode | [VoiceActivityDetectionMode](#voiceactivitydetectionmode) | Выбор типа разметки VAD-ом аудиофайла для запроса в файловом режиме. |
...
//Окончание конфига запроса по распознаванию речи


//Начало конфига ответа на запрос по распознаванию речи

#### FileRecognizeResponse

Ответ с результатами распознавания для метода [FileRecognize](#filerecognize).

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| response | repeated [RecognizeResponse](#recognizeresponse) | Результат распознавания. |
...
| Поле | Тип | Описание |
| :--- | :--- | :--- |
| hypothesis | [SpeechRecognitionHypothesis](#speechrecognitionhypothesis) | Результат распознавания речи. |
...
| genderage | [SpeakerGenderAgePrediction](#speakergenderageprediction) | Результат работы модели классификации мужчина/женщина/ребенок. Включается флагом enable в [GenderAgeEmotionConfig](#genderageemotionconfig). |
| spoofing_result | repeated [SpoofingResult](#spoofingresult) | Результат работы модели антиспуфинга. Включается флагом enable в [AntiSpoofingConfig](#antispoofingconfig). |
| speaker_info | [SpeakerInfo](#speakerinfo) | Информация по спикеру, которому принадлежат распознанные фразы. |
...
| words | repeated [WordInfo](#wordinfo) | Результат распознавания речи с разбивкой по словам. |
| normalized_words | repeated [WordInfo](#wordinfo) | В настоящее время не поддерживается. |
...
| Поле | Тип | Описание |
| :--- | :--- | :--- |
| type | [AttackType](#attacktype) | Тип спуфинг-атаки. |
| result | [AttackResult](#attackresult) | Результат определения является ли звонок спуфинг-атакой. |
...
//Окончание конфига ответа на запрос по распознаванию речи


//Начало информации о моделях распознавания речи.
...
//Окончание информации о моделях распознавания речи.

## Асинхронное (отложенное) распознавание речи

Взаимодействие с API отложенного распознавания речи осуществляется по протоколу gRPC.

**Примечание**: Подробности об этом протоколе можно прочитать на https://grpc.io/

Чтобы пользоваться сервисом **Audiogram** для отложенного распознавания речи нужно создать клиентское приложение. Можно использовать любой язык программирования, который есть в библиотеке для работы с gRPC.

При написании приложения используйте [proto-файл **longrunning_stt.proto**](https://github.com/mts-ai/audiogram/blob/main/asr/v3/longrunning_stt.proto).
...
| Имя метода | Тип запроса | Тип ответа | Описание |
|:-----------|:------------|:-----------|:---------|
| LongRunningRecognize | LongRunningRecognizeRequest | Task | Выполняет распознавание речи в файловом режиме (асинхронном). Подходит для распознавания речи в крупных аудиофайлах, но может занимать некоторое время. |

#### GetTaskInfo

| Имя метода | Тип запроса | Тип ответа | Описание |
|:-----------|:------------|:-----------|:---------|
| GetTaskInfo | TaskRequest | Task | Получает информацию по задаче на распознавание речи. |

#### CancelTask

| Имя метода | Тип запроса | Тип ответа | Описание |
|:-----------|:------------|:-----------|:---------|
| CancelTask | TaskRequest | Task | Отменяет задачу на распознавание речи. |

### Сообщения PROTOBUF для отложенного распознавания речи в файловом режиме (асинхронном)

#### Task

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| id | string | Идентификатор задачи на распознавание речи в файловом режиме (асинхронном). |
| status | Status | Статус задачи на распознавание речи. |
| created_at | google.protobuf.Timestamp | Время создания задачи на распознавание речи. |
| audio_requests | repeated Audio | Информация по запросам, входящим в задачу на отложенное распознавание аудио. |
...
| Поле | Тип | Описание |
| :--- | :--- | :--- |
| bucket_name | string | Название s3-контейнера (бакета), в котором хранится аудиофайл, речь из которого необходимо распознать. |
| object_name | string | Название объекта (в состав объекта входит аудиофайл и метаданные), речь из которого необходимо распознать. |
| status | string | Статус задачи на распознавание речи. |
| response_id | string | Идентификатор ответа на запрос на распознавание речи. |

#### TaskRequest

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| task_id | string | Идентификатор задачи на распознавание речи. |

#### LongRunningRecognizeRequest

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| config | mts.ai.audiogram.stt.v3.RecognitionConfig | Конфигурация распознавания речи. |
| s3_audio_path | repeated AudioPath | Путь к аудиофайлу в s3, речь из которого надо распознать. |

#### AudioPath

| Поле | Тип | Описание |
| :--- | :--- | :--- |
| bucket_name | string | Название s3-nконтейнера (бакета), в котором хранится объект с аудиоданными на распознавание. |
| object_name | string | Название объекта (аудиофайла и метаданных ), речь из которого необходимо распознать. |
...
При написании приложения используйте [proto-файл **tts.proto**](https://github.com/mts-ai/audiogram/blob/main/tts/v2/tts.proto).
...
| POST_PROCESSING_PHONE_CHANNEL | 1        | Этот параметр **поддерживается только моделью синтеза light**. Он может улучшить качество звучания аудио, но отрицательно влияет на скорость синтеза.<br><br> Если качество звучания устраивает без этого параметра, лучше его не применять.<br><br> Допускается использовать только с SynthesizeSpeechRequest.sample_rate_hertz = 8000 Гц. Наилучший результат дает в сочетании с SynthesizeOptions.model_sample_rate_hertz = 22050 Гц. |
...
| :------------     |:---------  | :--------               | :------------------------------|
| text  | string |  | Текст для синтеза без SSML разметки<br> (необходимо задавать только одно из полей – text или ssml).<br> Если в поле text отправить на озвучку текст с SSML-тегами, Audiogram озвучит не только текст, но и теги.      || ssml             | string    |                 | Текст для синтеза в формате SSML<br> (необходимо задавать только одно из полей – text или ssml).<br> Если в поле ssml отправить на озвучку текст без SSML-разметки, вернется ошибка синтеза.|
...
## Текущая версия - Audiogram 3.27.0

Обновление Audiogram 3.27.0 содержит следующие улучшения и изменения:
